...
import {
    ...
    Wand2, Zap, CheckCircle, XCircle, GripHorizontal, AlertTriangle, QrCode, Target, Loader2, MapPin, Copy, Eye, PenTool, Trophy, Download, RefreshCw
} from 'lucide-react';
...
const PlaygroundEditor: React.FC<PlaygroundEditorProps> = ({
    ...
    const [editingIconType, setEditingIconType] = useState<'standard' | 'incorrect' | 'completed' | null>(null); // Track which icon section is being edited
    ...
    const activePlayground = uniquePlaygrounds.find(p => p.id === activePlaygroundId) || uniquePlaygrounds[0];
    ...
    const handleSnapAllToGrid = () => {
        ...
        snappedPoints.forEach(sp => {
            positionUpdates[sp.id] = {
                x: Math.round(sp.devicePositions?.[selectedDevice]?.x || 50),
                y: Math.round(sp.devicePositions?.[selectedDevice]?.y || 50)
            };
        });
                    ...
                    >
                        ...
                        {uniquePlaygroundPoints.map((point, index) => {
                            ...
                            return (
                                <div
                                    key={point.id}
                                    className={`absolute transform -translate-x-1/2 -translate-y-1/2 touch-none select-none group ${
                                        isSimulationActive ? 'cursor-pointer'
                                        : isDraggingThis ? 'cursor-grabbing z-50'
