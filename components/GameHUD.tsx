...
const MAP_STYLES_LIST: { id: MapStyleId; label: string; icon: any; preview?: string; className?: string }[] = [
    ...
    { id: 'ski', label: 'Ski Map', icon: Snowflake, preview: 'https://a.tile.opentopomap.org/13/4285/2722.png' },
...
const GameHUD = forwardRef<GameHUDHandle, GameHUDProps>(({    accuracy, mode, toggleMode, onSetMode, onOpenGameManager, onOpenTaskMaster,
    ...
    authUser, activeGame, onUpdateGame, onStartSimulation, onToggleSnapToRoad, snapToRoadMode, onExecuteSnap, selectedSnapTaskCount = 0,
    ...
    return (
        <>
            ...
            <ToolbarsDrawer
                ...
                onExecuteSnap={onExecuteSnap}
                selectedSnapTaskCount={selectedSnapTaskCount}
                    ...
                    <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700 rounded-xl shadow-2xl p-1 cursor-move group relative">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex gap-1">
                                ...
                                {/* Danger Zone Button - INSTRUCTOR mode only */}
                                {mode === GameMode.INSTRUCTOR && onAddDangerZone && (
                                    <div className="flex flex-col items-center gap-0.5">
                                        ...
                                        <div className="text-[7px] font-black text-yellow-100 uppercase tracking-widest leading-tight whitespace-nowrap">DANGER</div>
                                    </div>
                                )}
            ...
            {/* Draggable PINS Toolbar (Measure, Scores, Relocate) - Only for GPS-based games - Hidden in Instructor Mode */}
            {mode !== GameMode.INSTRUCTOR && (mode === GameMode.EDIT || mode === GameMode.PLAY) && activeGame?.gameMode !== 'playzone' && visibleToolbars['pins'] && (
                <div
                    className="absolute z-[1100] pointer-events-auto touch-none"
