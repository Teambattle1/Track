...
// Main collapsible section for top-level drawer organization (Playzones, Tasks, Save)
const MainSection = ({
    title,
    icon: Icon,
    count,
    ...
    return (
        <div className="border-b border-gray-200 dark:border-gray-700">
            <button
                onClick={onToggle}
                className="w-full flex items-center justify-between p-3 bg-white dark:bg-gray-850 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-black uppercase text-gray-700 dark:text-gray-200 tracking-wide">{title}</span>
                    {count !== undefined && (
                        <span className="text-[10px] font-bold bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded text-gray-500">{count}</span>
                    )}
                    {badge}
                </div>
                <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${isCollapsed ? '-rotate-90' : ''}`} />
      ...
      {!activeGame ? (
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <MainSection
                title="Playzones"
                icon={Gamepad2}
                count={activeGame?.playgrounds?.length || 0}
                isCollapsed={playzonesCollapsed}
                onToggle={() => setPlayzonesCollapsed(!playzonesCollapsed)}
                color="purple"
            >
                <div className="p-3 space-y-3">
                    ...
                    {activeGame?.playgrounds && activeGame.playgrounds.length > 0 ? (
                                    ...
                                    <button
                                        ...
                                        className={`p-3 rounded-lg transition-all active:scale-[0.98] text-left group ${
                                            isHovered
                                                ...
                                                : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-orange-400 dark:hover:border-orange-600 hover:shadow-md'
                                        }`}
                                    >
                        ...
                        <div className="text-[10px] text-gray-400 italic text-center py-4">No playzones yet</div>
                    )}

                    {/* ADD PLAYZONE + SIMULATE buttons */}
                    <div className="flex gap-2">
                        {onOpenPlaygroundEditor && (
                            <button
                                onClick={() => onShowPlayzoneChoice?.()}
                                className="flex-1 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all flex items-center justify-center gap-2"
            ...
            <MainSection
                title="Tasks"
                icon={Map}
                count={mapPoints.length + playgroundGroups.reduce((sum, pg) => sum + pg.points.length, 0)}
                ...
                <div className="p-3">
                    ...
                    <SortableContext items={mapPoints.map(p => p.id)} strategy={verticalListSortingStrategy}>
                        <ZoneSection
                            id="zone-map"
                            title="ON MAP"
                            tooltip="Tasks placed on the map. Players must visit these GPS locations to unlock and complete them."
                            icon={Map}
                    ...
                    <div className="mb-2 rounded-xl">
                        <div
                            ...
                            className="flex items-center justify-between p-3 bg-gray-100 dark:bg-gray-800 rounded-xl cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors border border-gray-200 dark:border-gray-700"
                            title="Red zones that deduct points when players enter them. Use for off-limits areas or to add challenge."
                        >
                            <div className="flex items-center gap-2">
            ...
            <MainSection
                title="Save"
                icon={Save}
                isCollapsed={saveCollapsed}
                onToggle={() => setSaveCollapsed(!saveCollapsed)}
                color="green"
            >
                <div className="p-3 space-y-2">
                    {!isGameTemplateMode && onSaveGameTemplate && (
                        <button
                            ...
                            className="w-full py-2.5 font-bold rounded-lg transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wide bg-purple-600 text-white hover:bg-purple-700"
                        >
                            <Upload className="w-3.5 h-3.5" /> Save as Template
                        </button>
                    )}

                    <button
                        onClick={handleSaveClick}
                        className={`w-full py-2.5 font-bold rounded-lg transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wide ${isSaved ? 'bg-green-100 text-green-700 border-2 border-green-500' : (isGameTemplateMode ? 'bg-purple-600 text-white hover:bg-purple-700' : 'bg-green-600 text-white hover:bg-green-700')}`}
                    >
                        {isSaved ? (
                            <><Check className="w-3.5 h-3.5" /> {isGameTemplateMode ? 'Template Updated!' : 'Game Saved!'}</>
                        ) : (
                            <><Save className="w-3.5 h-3.5" /> {isGameTemplateMode ? 'Update Template' : 'Save Game'}</>
                        )}
                    </button>
                </div>
            </MainSection>
